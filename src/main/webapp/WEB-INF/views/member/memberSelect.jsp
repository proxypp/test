...
<script src="resources/jss/jquery-3.6.0.min.js"></script>
 <script>
 		$(document).ready(function(){
 			...
 			$('#btn1').click(function(){
 				if (confirm("수정하시겠습니까?")) {
 				document.form1.action = "memberUpdateForm.do";
 				document.form1.submit();
				}
 			});	
 		});
...
<body>
  ...
  <div id="main">
    <div class="product-container">
      ...
      <img src="resources/img/코드로 배우는 스프링.jpg" alt="">
    </div>
    <div class="card" style="height: 600px;">
      ...
      <form name="form1" method="post">
        <label for="number"><h1 style="color: blue">내 아이디</h1>
         	<div><h3>${member.id}</h3> </div>
        </label><br>
        <label for="name"><h1 style="color: blue">내 이름</h1>
         	<div><h3>${member.name}</h3> </div>
        </label><br>
        <label for="address"><h1 style="color: blue">내 주소</h1>
         	<div><h3>${member.address}</h3> </div>
        </label><br>
        <label for="tel"><h1 style="color: blue">내 전화번호</h1>
         	<div><h3>${member.tel}</h3> </div>
        </label>
        <label for="tel"><h1 style="color: blue">내 등급</h1>
         	<div><h3>${member.author}</h3> </div>
        </label>
        <label>
    		<input type="button" value="수정" id="btn1" name="btn2">&nbsp;&nbsp;&nbsp;
			<input type="button" value="회원탈퇴" id="btn2" name="btn2">
		</label>
    </form>
    </div>
  </div>
